...
from DSA_2_0.font_large import font_large
from DSA_2_0.font_small import font_small
...
custom_font_index = None

try:
    with open("/DSA_2_0/custom_font.json") as f:
        font_custom = json.load(f)

    for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        if c in font_custom:
            font_custom[c.lower()] = font_custom[c]

    if " " in font_custom:
        font_custom["_"] = font_custom[" "]
        font_custom["-"] = font_custom[" "]

    fonts.append(font_custom)
    custom_font_index = len(fonts) - 1

except Exception as e:
    print("custom_font not loaded:", e)
