...
def timer():
    html = web.timer(varinit.settings["timer"])
    return html
...
def mkhtml(checknet=0, version_message=False, reset_message=0):
    ...
    if checknet and not varinit.saving_state:
        functions.sysprint(functions.TXT["select_network"], 10, cls=topbottom, shading=True)
        functions.sysprint(functions.TXT["save"], 11, shading=False)

    if reset_message:
        functions.sysprint(functions.TXT["reboot"], 10, cls=topbottom)
...
def huvudsidan(request):
    ...
    try:
        varinit.settings["utc_offset"] = int(request.params.get("utc_offset", 1))
    except:
        varinit.settings["utc_offset"] = 1
    
    varinit.settings["summer_time"] = 1 if "summer_time" in request.params else 0
    
    if not request.params: varinit.saving_state = False
    else:
        settings_output_one = functions.TXT["saving"]
        num = varinit.screen_selector
        ...
        try:
            if request.params["sstring"] == "":
                ...
                settings_output_one = functions.TXT["searching"]
                settings_output_two = str(request.params["sstring"].replace("+"," "))
                settings_output_two = dicts.url_decode(settings_output_two)
                settings_output_two = settings_output_two[:20]
                datalist = []
                #datastr = ""
                datastr = (
                    """<option value="0">"""
                    + functions.TXT["select_station"]
                    + """</option>"""
                )
...
def huvudsidan(request):
    ...
    if "set_timer" in request.params:
        ...
        try: 
            ...
            station_decoded = dicts.url_decode(station_decoded)
    ...
    elif "siteid" in request.params:
        siteid = str(request.params["siteid"])
        siteid = dicts.url_decode(siteid)
        varinit.settings["stations"][num]["siteid"] = siteid
        varinit.settings["siteid"] = siteid
        varinit.use_cached_data = False
        functions.refresh()
        print("SiteID gesetzt:", siteid)
        return (200, {}, "")

    elif "api_provider" in request.params:
        varinit.settings["api_provider"] = int(request.params["api_provider"])
        varinit.use_cached_data = False
        functions.refresh()
        varinit.cached_departure_data = {}
        varinit.html_cache = ""
        functions.reset_scroll()
        return (200, {}, "")

    elif "layout" in request.params:
        varinit.settings["layout"] = int(request.params["layout"])
        varinit.use_cached_data = False
        functions.refresh()
        varinit.html_cache = ""
        functions.switch(_screen=False)   # optional: Anzeige direkt aktualisieren
        return (200, {}, "")

    elif "utc_offset" in request.params:
        try:
            varinit.settings["utc_offset"] = int(request.params["utc_offset"])
        except:
            varinit.settings["utc_offset"] = 1
        functions.refresh()
        return (200, {}, "")
    
    elif "summer_time" in request.params:
        varinit.settings["summer_time"] = 1 - int(varinit.settings.get("summer_time", 0))
        return (200, {}, "")
    
    elif "ssid" in request.params:
        varinit.settings["ssid"] = request.params["ssid"]
        varinit.settings["ssid"] = dicts.url_decode(varinit.settings["ssid"])
        return (200, {}, "")
    elif "user" in request.params:
        varinit.settings["user"] = request.params["user"]
        varinit.settings["user"] = dicts.url_decode(varinit.settings["user"])
        return (200, {}, "")
    elif "password" in request.params:
        varinit.settings["password"] = request.params["password"]
        varinit.settings["password"] = dicts.url_decode(varinit.settings["password"])
        #functions.sysprint("Verbinde:", 10, cls=topbottom)
        ...
        if request.params["no_more_departures"] == "":
            varinit.settings["no_more_departures"] = "Keine weiteren Abfahrten"
        else:
            varinit.settings["no_more_departures"] = str(request.params["no_more_departures"])
        varinit.settings["no_more_departures"] = dicts.url_decode(varinit.settings["no_more_departures"])
        ...
        varinit.settings["mins"] = dicts.url_decode(varinit.settings["mins"])
        functions.switch(_screen=False)
        return (200, {}, "")
...
def search_station(request):
    ...
    datastr = '<option value="0">' + functions.TXT["select_station"] + '</option>'
    try:
